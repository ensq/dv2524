...
% C & C++
% Could cache data
% Caching data (vertex buffer data)

% Consider removing:
\section{Host System Renderer}
\label{sec:methodologysolution_hostsystemrenderer}
\index{Host System Renderer}


% Effectively being a part of the host system libraries (see...), it may be of importance to distinguish the...
% Due to some of the design desicions inherent in the opengl es 2.0 framework, and in consideration to advanced simulatory attributes such as checkpointing and reverse execution, the host-side of the paravirtualized framework need to maintain a set of variables ...
% This may need be extended... yadda yadda yadda
...
% Pre-initialized window (could easily be extended to tune in on EGL requests)
