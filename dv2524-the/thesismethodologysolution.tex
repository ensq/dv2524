...
Furthermore, and again similarily to the target system libraries, due to some design decisions inherent in the \dvttermopenglestwopointo\ framework, the host system libraries need maintain some data; in particular - vertex attributes.
...
However, due to the inhererent performance demands brought on by serialization of real-time graphics invocations, the methodology of \dvttermmagicinstruction s were selected for use for the purpose of this study.
...
However, other methodologies, albeit slower, may be fast enough to for use in graphics paravirtualization; the \dvttermandroidemulator , in addition to \dvttermmagicinstruction s, accomodating for TCP/IP communication\footnote{It may be of interest to know that the \dvttermsimics\ \dvttermopengles\ paravirtualization technology, during development, was accomodated by a TCP/IP client-/server model.}.
...
As such, the physical addresses of memory pages must be continiously retrieved and translated on a per-page basis; effectively 'traversing' the virtual memory table.
