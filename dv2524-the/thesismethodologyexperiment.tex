...
In order to accomodate for similar acceleration in \dvttermqemu , \dvttermandroid\ is configured to run on the \dvttermintel\ \dvttermandroid\ \dvttermxeightysix\ system image, circumventing the need to interpret \dvttermarm\ instructions (see \dvtcmdciteref{technicaldocs:intel:2013:x86}).
Furthermore, the solution utilizes \dvttermhaxm\ for similar native execution on the \dvttermhost\ hardware (see \dvtcmdciteref{technicaldocs:intel:2013:haxm}).
