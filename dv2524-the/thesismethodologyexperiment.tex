...
Furthermore, the solution also utilize \dvttermkvm\ in order to provide similar native execution on the \dvttermhost\ hardware\footnote{Had the solution been running on the \dvttermwindows\ \dvttermos , the solution would have utilized \dvttermhaxm\ (see \dvtcmdciteref{technicaldocs:intel:2013:haxm}) to accomodate for said native execution\dvtcmdciteref{web:hofemeier:2010}.}.
...
In the presence of of the complexities caused by the occurence of virtual time, profiling of time in virtual platforms sometimes dictate special measures.
...
In a \dvttermsimics\ simulation, as described by section \dvtcmdrefname{sec:background_virtualtime}, time fleets differently than in the context of the \dvttermhost\ machine.
...
If not specified otherwize, all results collected from the \dvttermsimics\ platforms are subtracted with the average of this profiling cost; being \dvtcmdfirstline{profile.dat.avg}\footnote{In some cases, where the overhead cost is to be considered substantial, approximatative data is presented accordingly (see section \dvtcmdrefname{sec:appendixa_magicinstructionprofiling} under \dvtcmdrefname{cha:appendixa}).}.\\
...
The benchark kernel, in it's entirety, is presented in section
...
The Phong benchmark is comprised of a rotating model, being the Newell teapot\todo{point out as to why the newell teapot is a standard model}, which is textured and subsequently shaded by a single point light using the Phong shading model; thus giving the benchmark it's name.
