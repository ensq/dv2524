...
The \dvttermopengles\ set of \dvttermapi s is developed by \dvttermkhronos , the same konsortium responsible for the development of the \dvttermopengl\ \dvttermapi s, which - unlike \dvttermopengl\ for embedded systems - intended for desktop graphics~\dvtcmdcitebib{publications:wright:2010}.
...
\dvttermopenglestwopointo\ consists of two \dvttermkhronos\ specifications; being the \dvttermopenglestwopointo\ \dvttermapi\ specification and the \dvttermopengles\ Shading Language Specification (\dvttermopengles\ SL)~\dvtcmdcitebib{publications:munshi:2008}.
Being derived from the \dvttermopengl ~$2.0$ specification~\dvtcmdcitebib{publications:wright:2010}, \dvttermopenglestwopointo\ supports programmable shaders and is no longer encumbered by the fixed functionality that characterized earlier versions of the \dvttermopengl\ and \dvttermopengles\ \dvttermapi s\dvtcmdcitebib{publications:munshi:2008}; making the \dvttermapi\ a modern contestant amongst a variety of applications on a range of platforms, such as \dvttermandroid\ and \dvttermios ~\dvtcmdcitebib{publications:wright:2010} popular on modern smartphones.
...
The continous need for faster processing units, slowed down by the heat dissipation issues limiting clock frequency since 2003~\dvtcmdcitebib{publications:kirk:2010}, has driven the utilization of \dvttermgpu s for general purpose workloads.
